...
void loop() {
    unsigned int alarm = readAnalog();
    bool fire = false;
    
    if(alarm > 100 && alarm < 450){ //smoke is detected.
      fire = true;   //there is fire.
    }
    
    while(fire == true){ //if there is fire then preform alarm.
      setIOOutput(3);
      delay(500);
      setIOOutput(0);
      delay(500);
      ...
      if (readIO() == 15){ //click on the button to stop the alarm
        fire = false;     //no fire anymore
        setIOOutput(0);
      }
    }
}
...
void setIOOutput(unsigned int value) {  //set in DDR the outputs 
  Wire.beginTransmission(0x38);
...
unsigned int readIO() {           //read inputs
  Wire.beginTransmission(0x38);
...
unsigned int readAnalog(){        //read analog signal
  Wire.requestFrom(0x36, 4);      //ID and number of bytes
  unsigned int AnalogInput0 = Wire.read()&0x03;  
  AnalogInput0 = AnalogInput0<<8;
  AnalogInput0 = AnalogInput0|Wire.read(); 
